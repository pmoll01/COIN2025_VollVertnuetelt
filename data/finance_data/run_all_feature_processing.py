...
# Zielwert zentral festgelegt
target_value = "sp500"

# Mapping: Skript → CLI-Argumentliste mit Platzhalter
scripts_with_args = {
    ...
    "data/finance_data/5_feature_atr_processing.py": ["--assets", "{target}"],  # Spezialfall
...
#    "data/finance_data/10_feature_cci_processing.py": ["--assets", "{target}"], #gelöscht
...
for script, arg_list in scripts_with_args.items():
    parsed_args = [arg.replace("{target}", target_value) for arg in arg_list]
