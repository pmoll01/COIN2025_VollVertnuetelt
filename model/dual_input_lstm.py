...
RESULT_CSV      = Path("results_fin_vs_fin_twitterv2.csv")
FEATURE_IMP_DIR = Path("feature_importance_all_scenarios_v5_WITH_TWITTER_DATA")
        ...
        for target in TARGETS:
            ...
            for phase in PHASES:
                ...
                for epoch in range(1, EPOCHS + 1):
                    ...
                    val_losses.append(np.mean(vlos))
                    scheduler.step(val_losses[-1])
                    lrs.append(optimizer.param_groups[0]["lr"])

                    tag = "fin+tw" if use_twitter else "fin_only"
                    print(f"{tag} | {asset}/{target}/{phase} | "
...
# BASE            = Path("Data/combined_pipeline_outputs")
# RESULT_CSV      = Path("results_all_scenarios_v4_WITH_TWITTER_DATA.csv")
# FEATURE_IMP_DIR = Path("feature_importance_all_scenarios_v4_WITH_TWITTER_DATA")
...
#                 fname = f"{asset}{target}_{{split}}_{phase}.csv"
#                 def load_df(split):
#                     path = BASE / fname.format(split=split)
#                     return pd.read_csv(path).reset_index(drop=True)

#                 # — Train, Val und Test laden —
#                 df_train = load_df("train")
#                 df_val   = load_df("val")
#                 df_test  = load_df("test")
...
#                 model = DualInputLSTM(
#                     num_finance_features=ds_train.X_seq.shape[2],
#                     num_tweet_features=   ds_train.X_tweet.shape[1],
#                     hidden_dim=64
#                 )
#                 loss_fn   = nn.MSELoss()
#                 optimizer = torch.optim.Adam(model.parameters(), lr=LR)
...
#                 train_loss_list, val_loss_list, lr_list = [], [], []
...
#                     model.eval()
#                     with torch.no_grad():
#                         train_losses = [loss_fn(model(xs, xt), yv).item()
#                                         for xs, xt, yv in dl_train]
#                         val_losses   = [loss_fn(model(xs, xt), yv).item()
#                                         for xs, xt, yv in dl_val]
#                     train_loss = np.mean(train_losses)
#                     val_loss   = np.mean(val_losses)

#                     # Scheduler mit Val-Loss füttern
...
#                     print(
#                         f"Asset={asset} Target={target} Phase={phase} | "
#                         f"Epoch {epoch:03d} | "
...
#                 plt.xlabel("Epoch"); plt.ylabel("LR"); plt.title("LR Schedule"); plt.legend()
#                 plt.tight_layout()
...
#                         drops.append(base_acc - accuracy_score(y_test, preds_perm))
#                     feature_importances.append((fname, np.mean(drops)))
